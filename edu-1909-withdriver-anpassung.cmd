...
if /i "%~1"=="" ( set TARGETIMAGE=%~d0\2_IMAGES\edu-1909-%today%-drivers.wim ) else ( set TARGETIMAGE=%~1 )
set PATCHESPATH=%~d0\4_WindowsUpdateKatalog\Updates\Windows10-x64\General\18362
set DRIVERBASE=%~d0\5_Treiber
...
rem check if driver dir exists
if not exist %DRIVERBASE% (
  echo ERROR: Dirver base directory must exist.
  goto fail
)
...
DEL /Q "%MOUNTDIR%\*.MRK"
echo %today% > "%MOUNTDIR%\EDU-1909-%today%.MRK"
echo = patches ======================= >> "%MOUNTDIR%\EDU-1909-%today%.MRK"
echo %PATCHES% >> "%MOUNTDIR%\EDU-1909-%today%.MRK"
echo = driver ======================== >> "%MOUNTDIR%\EDU-1909-%today%.MRK"
dir /s /A:D /B "%DRIVERBASE%" >> "%MOUNTDIR%\EDU-1909-%today%.MRK"
echo.
...
:drivers
rem
rem driver integration
rem
echo *** Integrate drivers ***
dism /English /Image:%MOUNTDIR% /Add-Driver /Driver:"%~d0\5_Treiber" /Recurse
...
if errorlevel 1 (
  ...
  echo %IMAGEX% /INFO %TARGETIMAGE% !LastNumberMount! "Windows 10 Education drivers %today%" "[Script] Anpassungen & Patches, Drivers Esprimo"
  if exist %IMAGEX% (
    %IMAGEX% /INFO %TARGETIMAGE% !LastNumberMount! "Windows 10 Education drivers %today%" "[Script] Anpassungen & Patches, Drivers Esprimo"
  )
)
