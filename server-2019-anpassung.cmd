@ECHO OFF
title Windows Server 2019 image creation
...
echo ************************************************************
rem created by Martin Aulenbach 11/26/2019
...
rem
set BASEWIM=%~d0\2_IMAGES\WIM\server-2019-base.wim
if /i "%~1"=="" ( set TARGETIMAGE=%~d0\2_IMAGES\server-2019-%today%.wim ) else ( set TARGETIMAGE=%~1 )
...
echo %today% > "%MOUNTDIR%\SERVER-2019-%today%.MRK"
echo %PATCHES% >> "%MOUNTDIR%\SERVER-2019-%today%.MRK"
echo.
...
choice /T 10 /D N /C YN /M "Apply default registry settings? default: no"
...
echo.
echo  Examples:
echo    %~n0 C:\winsetup\server-2019-install.wim
echo.
goto cleanup
