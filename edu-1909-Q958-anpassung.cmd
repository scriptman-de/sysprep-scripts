...
echo.
echo ************************************************************
echo ***           Windows 10 Education 1909 Q958             ***
...
if /i "%~1"=="" ( set TARGETIMAGE=%~d0\2_IMAGES\edu-1909-Q958-%today%.wim ) else ( set TARGETIMAGE=%~1 )
...
DEL /Q "%MOUNTDIR%\*.MRK"
echo %today% > "%MOUNTDIR%\EDU-1909-Q958-%today%.MRK"
echo %PATCHES% >> "%MOUNTDIR%\EDU-1909-Q958-%today%.MRK"
echo.
...
:drivers
rem
rem driver integration Q958
rem
echo *** Integrate Q958 drivers ***
dism /English /Image:%MOUNTDIR% /Add-Driver /Driver:"%~d0\5_Treiber\drivers-Q958-FTS" /Recurse
...
if errorlevel 1 (
  ...
  echo %IMAGEX% /INFO %TARGETIMAGE% !LastNumberMount! "Windows 10 Education Q958 %today%" "[Script] Anpassungen & Patches, Drivers Esprimo Q958"
  if exist %IMAGEX% (
    %IMAGEX% /INFO %TARGETIMAGE% !LastNumberMount! "Windows 10 Education Q958 %today%" "[Script] Anpassungen & Patches, Drivers Esprimo Q958"
  )
)
